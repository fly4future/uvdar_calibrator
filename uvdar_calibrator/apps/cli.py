...
Photos are fed one at a time into engine.calibrator.Calibrator.
Similar images are rejected on purpose so the accepted calibration set is diverse.

Fast center search is always used.
Plots are shown by default unless --no_plots is used.
"""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--image_dir",
        default="photos",
        help="Folder containing calibration images. Default: photos.",
    )
    ...
    p.add_argument(
        "--output_dir",
        default=".",
        help="Folder for Omni_Calib_Results, calib_results.txt, and reports.",
    )

    p.add_argument(
        "--no_plots",
        action="store_true",
        help="Run calibration/export without showing diagnostic plots.",
    )

    p.add_argument(
        "--skip_find_center",
        action="store_true",
        help="Skip the fast Find Center step for a quick test run.",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Only feed images through sample selection and print the readiness report; "
            "do not calibrate."
        ),
    )
...
def run(
    ...
    Loads images from a folder, runs sample selection, optionally calibrates,
    then saves/export results.
    """
    ...
    if not files:
        ...
        print("  python -m uvdar_calibrator --image_dir photos")
    ...
    print(
        ...
        "This is expected and helps produce a diverse calibration set."
    )
    ...
    if not cal.goodenough:
        print(
            "\nWARNING: sample variety is not yet marked ready. "
            "Calibrating anyway. Treat the result as preliminary and capture "
            "the suggested missing views if needed."
        )
